...
#
# ANCHORED to the start-of-line HTML-comment form the generator actually
# emits — NOT a loose substring match. A loose match self-defeats: the
# page that DOCUMENTS this exclusion has to name the marker in prose,
# and a substring match would exclude that whole page from the scan,
# silently blinding gate 3 to every offender on it. That is not
# hypothetical — it happened here, hiding two live offenders on
# docs/reference/convention-drift-detector.md until the raw grep count
# disagreed with the gate. Same mid-prose-vs-anchored-line distinction
# loom-2y6x locked for the `AUTOFAN-EXCLUDE:` marker.
GATE3_GEN_MARKER_RE='^<!-- GENERATED by scripts/loom-docs-gen'
...
gate3_scan() {
  ...
  while IFS= read -r f; do
    [ -f "$f" ] || continue
    grep -qE "$GATE3_GEN_MARKER_RE" "$f" 2>/dev/null && continue
    cdg_bare_scan "$f"
...
{
  printf '# loom-guest\n\n'
  printf '<!-- GENERATED by scripts/loom-docs-gen — DO NOT EDIT. Edit the source primitive, then rerun the generator. -->\n\n'
...
echo "==> GATE 3 case E (RED control): a page that MENTIONS the generator marker mid-prose is still SCANNED"
G3F="$(mktemp -d)"
mkdir -p "$G3F/docs/reference"
{
  printf '# convention drift detector\n\n'
  printf 'Generator-produced pages carry a `GENERATED by scripts/loom-docs-gen`\n'
  printf 'marker and are excluded from the scan.\n\n'
  printf 'A how-to that says "run `scripts/loom-rebase-worktree main`" is wrong.\n'
} > "$G3F/docs/reference/convention-drift-detector.md"
g3f_off="$(gate3_scan "$G3F")"
if [ -n "$g3f_off" ]; then
  pass "gate3: mid-prose marker mention does NOT exclude the page (anchored match)"
else
  fail "gate3: mid-prose marker mention does NOT exclude the page (anchored match)" \
    "(gate3_scan returned empty — a page merely NAMING the marker excluded ITSELF from the scan,
silently blinding the gate to every offender on it. The marker match must be ANCHORED to the
start-of-line HTML-comment form the generator emits, per loom-2y6x.)"
fi
rm -rf "$G3F"

echo "==> GATE 3 case F (LIVE): the real docs/ tree has NO bare GLOBAL_ONLY reference"
