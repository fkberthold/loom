...
| `loom-drift-nudge.sh` | SessionStart | n/a | Non-blocking (always exit 0); for a loom-managed project (`.claude/workflow.json` OR an existing stamp) emits a one-time-per-session stderr nudge pointing at `/audit-project --apply-drift` in either of two states — **never synced** (no `.claude/.loom-sync` stamp at all, or a stamp carrying only `last_checked` with no `last_synced` record) or **stale** (stamped `last_synced` hash differs from loom's current `scripts/loom-convention-manifest` hash). Compares `last_synced` ONLY, so a read-only `/audit-project --check=` can never silence it (loom-uh4i); a legacy `hash=` stamp is grandfathered as `last_synced` | `LOOM_DRIFT_NUDGE_SKIP=1` (literal "1"); loom-managed by NEITHER signal → silent no-op; matching hash → silent; stamp with neither record → fail-open silent; subagent payloads skip |
| `main-checkout-edit-guard.sh` | PreToolUse | `Edit\|Write\|MultiEdit` on a source/test file (per the shared `loom_is_source_or_test` classifier) that resolves inside the MAIN checkout while a bead is in_progress — the inverse of `edit-write-pwd-guard.sh`, which is inert from main by design | Blocks (exit 2) naming the expected worktree path + `git worktree add` recovery command | `LOOM_MAIN_CHECKOUT_GUARD_SKIP=1` (literal-1 match); cwd in a linked worktree, `*.md`/`docs/`/`*.json` targets, target outside main or already under `.worktrees/`/`.claude/worktrees/`, no `bd`, and no in_progress bead all pass through |
