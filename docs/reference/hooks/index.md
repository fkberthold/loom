...
| `loom-drift-nudge.sh` | SessionStart | n/a | Non-blocking (always exit 0); compares a managed project's stamped `.claude/.loom-sync` hash against loom's current `scripts/loom-convention-manifest` hash → one-time-per-session stderr nudge pointing at `/audit-project --apply-drift` on drift | `LOOM_DRIFT_NUDGE_SKIP=1` (literal "1"); no `.claude/.loom-sync` stamp → silent no-op; subagent payloads skip |
