...
def test_kg_stats_returns_expected_keys(dolt_server_env):  # noqa: F811 - pytest fixture param, not a redefinition
    """kg_stats() must return the expected shape whether or not the
    sibling loom-40ec.4.3 tools/kg.py has landed."""
    result = kg_stats()
    assert set(result.keys()) == {"entity_count", "triple_count"}
