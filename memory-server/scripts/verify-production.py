...
Round-trips each memsrv_* tool against the REAL production Dolt
...
path (memsrv_add_drawer) is deliberately NOT exercised here; the
...
def main() -> int:
    print(f"Verifying production memsrv against "
          f"{os.environ.get('LOOM_MEMORY_HOST', '127.0.0.1')}:"
          f"{os.environ['LOOM_MEMORY_PORT']}\n")

    # 1. memsrv_status — real migrated row counts.
    st = status.status()
    check(
        "memsrv_status.dolt_reachable",
        st["dolt_reachable"] is True,
        f"dolt_reachable={st['dolt_reachable']}",
    )
    check(
        "memsrv_status.total_drawers > 0",
        st["total_drawers"] > 0,
    ...
    check(
        "memsrv_kg_query('loom_repo') returns facts",
        len(facts) > 0,
