...
- **light** — run an abbreviated routine: `bd stats` + `bd ready -n 10` +
  ...
  + `bd list --status=in_progress --limit 0` + reconcile + pick. Skip palace status/KG-stats/diary-deep-dive (steps 3-4)
...
1a. **Surface in-progress work first — "RESUME?" header.** Run `bd list --status=in_progress --json --limit 0` and parse each entry's id, title, and updated/last-touched timestamp. For each in_progress bead, also surface a recency cue from the last diary entry (via `mempalace_diary_read("claude-opus", 3)` or project-agent-wing equivalent) when the diary timestamp lines up with the bead's last_touched window. Print a single block:
   ...
   Resume cues outrank ready-bead selection — a half-finished bead is almost always the right next move (loom-z3m.1 f4: liza_base-n7pb sat in_progress unnoticed across sessions until the user surfaced it manually). **Skip the header entirely if `bd list --status=in_progress --json --limit 0` returns an empty array** — no in_progress means nothing to resume; emitting an empty "RESUME?" block on every cold-start is just noise. Tolerance: if `bd list` errors, emit `(bd in_progress check skipped)` and continue. **Never fail the skill on this step.**
...
1b. **Since you were last here — long-gap digest.** Detect time since the last user prompt: prefer the mtime of the latest `~/.claude/projects/<slug>/*.jsonl` transcript file; fall back to message-timestamp parsing inside that file. If the gap is **>3 days**, emit a "Since you were last here:" digest combining:
   ...
   - `bd list --status=closed --since="<gap_start>" --limit 0` — what shipped while away
...
1e. **Surface active explorations — "ACTIVE EXPLORATION" header.** An *exploration* (opened via `/explore <idea>`) is a NEW above-bead, SUB-design primitive — the permissive front-door to `/design-a-cycle`, with NO soundness gate. Like a design cycle, an exploration is **not a bead** and has no single RED→GREEN, so `bd ready` / `bd list --status=in_progress` will never surface it. Its memory is ONE drawer in the project's `<wing>/decisions` room (loom's own lives in wing `loom`, room `decisions`), tagged `exploration` (tag-not-room), carrying a STATUS of `active` | `rested` | `promoted`. Discover any *active* exploration so a cold-start resumes the thinking, not just the bead queue. <!-- bd-unbounded-ok: statement about what the query does not surface; no invocation to bound -->

   **Real tag-based discovery (loom-40ec.4.5.2, shipped 2026-07-10).** Exploration drawers are discovered via `mempalace_search(query=..., wing=<wing>, room="decisions", tag_filter=["exploration"])` — the `exploration` tag (applied via `mempalace_tag_drawer` at filing time; see the `explore` skill's Discovery section) is the authoritative membership signal, not a title/body-text convention. All exploration drawers that predated real tagging were backfilled with the `exploration` tag as part of loom-40ec.4.5.6 (2026-07-11); there is no title/marker fallback to maintain.

   Read the STATE HEADER's `status` field on each tag-matched drawer to filter further — the tag answers "is this an exploration drawer," `status` answers "is it currently active." For each active exploration, print one line:
...
1g. **Convention-drift nudge (loom-ig3p.3).** A SessionStart hook, `hooks/loom-drift-nudge.sh`, already ran before this skill's steps and may have printed a one-line stderr nudge — this step is just "surface it if it fired," parallel to 1f's constitution fingerprint but for loom's own shipped conventions rather than the project's local tooling profile. The hook fires for a LOOM-MANAGED project — one carrying `.claude/workflow.json`, or an existing `.claude/.loom-sync` stamp (only `/audit-project` writes one, so it is itself proof of opt-in) — in either of two states. **Stale stamp:** the project's stamped `last_synced` hash (`<project>/.claude/.loom-sync`, written by `scripts/loom-sync-stamp` at the last `/audit-project` run that actually APPLIED remediation) differs from loom's CURRENT `scripts/loom-convention-manifest` hash, meaning the project's `templates/`-derived scaffolds (docs substrate, design-doc drawers, exploration drawers, `.claude/project-constitution.md`) may have drifted from what loom ships today. **Never synced:** the project is loom-managed but carries no `last_synced` record — either no stamp at all, or a stamp carrying only the `last_checked` field a read-only audit writes — so it has never received loom's conventions, which is the state that most needs surfacing (loom-oktm; before that fix this case was silently swallowed). The hook compares `last_synced` ONLY: merely running `/audit-project --check=...` records a check and can never silence the nudge (loom-uh4i — before that fix, a read-only audit stamped loom's current hash and the detector could be quieted by LOOKING at it). Both emit a `[loom-drift-nudge] INFO: ...` line, worded differently — the never-synced one names the absent stamp and asks for a first sync, the stale one names both hashes and the sync date and asks for a resync. If either line appears in this session's startup output, surface it to the user as a single line and mention `/audit-project --apply-drift` as the fix; do not re-derive or re-check the drift yourself. **Silent when the hook didn't fire** — a project that is loom-managed by NEITHER signal, or one whose stamp matches loom's current hash, produces no output, so there is nothing to surface. **Never fail the skill on this step.**
1h. **Liza's complaints notebook (liza_base only).** In the `liza_base` project, Liza files complaints from inside her own session via `/gripe` (bd `liza_base-zlp3`) — the notebook exists so that something coming back wrong costs her a sentence instead of climbing out of a scene to become a bug report. Nothing surfaces that queue unless this step does. If the project is `liza_base` and `scripts/triage_complaints.py` exists, run from the repo root:

    ```bash
    devbox run python -m scripts.triage_complaints list 2>/dev/null || true
    ```

    Surface the header line and any `>>` rows — threads where she wrote last, ours to answer. The `..` rows are threads we already asked back in and are waiting on HER; those are not actionable work and must not be presented as a queue. Skip silently if the command fails, prints nothing, or reports nothing waiting — **this step must never block or fail session startup.**

    **Invocation is load-bearing:** use `devbox run python -m scripts.triage_complaints`, NOT `python scripts/triage_complaints.py`. The plain form is refused by the `constitution-enforce` hook (any bare `python`/`python3` program token, including the venv's own interpreter), and the by-path form dies on `ModuleNotFoundError: No module named 'liza_base'` because Python sets `sys.path[0]` to `scripts/` rather than the cwd. Since this step ends in `|| true`, a wrong invocation is SWALLOWED — the notebook would look permanently empty at every session start with no error to notice.

    **Do NOT triage automatically:** do not create beads, infer priorities, or run a model over the complaints. Locked 2026-07-19 (Frank) — the queue is for a person to read, by someone who can see the rest of the backlog and not duplicate it. Offer `reply` / `handled --bead` as next steps only if asked. Mirrored in `docs/how-to/triage-complaints.md` in the liza_base repo; keep the two in sync.

2. **Check in-progress.** Run `bd list --status=in_progress --limit 0`. Anything there outranks the ready queue — finish what was started.
