...
1e. **Surface active explorations — "ACTIVE EXPLORATION" header.** An *exploration* (opened via `/explore <idea>`) is a NEW above-bead, SUB-design primitive — the permissive front-door to `/design-a-cycle`, with NO soundness gate. Like a design cycle, an exploration is **not a bead** and has no single RED→GREEN, so `bd ready` / `bd list --status=in_progress` will never surface it. Its memory is ONE drawer in the project's `<wing>/decisions` room (loom's own lives in wing `loom`, room `decisions`), tagged `exploration` (tag-not-room), carrying a STATUS of `active` | `rested` | `promoted`. Discover any *active* exploration so a cold-start resumes the thinking, not just the bead queue.
   ...
   **INTERIM FALLBACK — `mempalace_tag_drawer` unavailable (loom-40ec.4.5).** The natural mechanism to tag and filter exploration drawers — `mempalace_tag_drawer` with a `tag_filter` search param — is not yet available on the currently-loaded MemPalace MCP tool surface (tracking bead loom-40ec.4.5). Until that capability ships, step 1e runs a documented FALLBACK: scan via `mempalace_search` (or `mempalace_list_drawers` for the project wing) for drawers in the project's `<wing>/decisions` room whose title starts with `# EXPLORATION` AND whose body contains the standardized machine-parseable status marker `<!-- tag: exploration status: active -->` (or `rested`/`promoted`). This fallback is interim and should revert to real tag-filtering once loom-40ec.4.5 ships.

   Scan via `mempalace_search` (or `mempalace_list_drawers` for the project wing) for drawers matching the title and status-marker convention — the same way step 1d scans DESIGN DOC drawers — and treat an exploration as **active** only when its status marker shows `active`. For each active exploration, print one line:
